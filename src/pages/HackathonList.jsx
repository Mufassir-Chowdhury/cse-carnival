...
import { hackathonSelectedTeams } from "../data/hackathonSelectedTeams";


const people = hackathonSelectedTeams;
  ...
  return (
    <div className=" pt-24">
      ...
      <Marquee gradient={false} speed={100} className=" overflow-y-hidden bg-navbar-button-hover">
        <p className="text-center text-md font-semibold py-1 text-white">**Hackathon Payment Deadline: 12th February, 2024** </p>
      </Marquee>
          ...
          {filteredPeople.map((person, index) => (
            <li key={person.url} className="flex justify-between odd:bg-slate-500 odd:bg-opacity-10 px-4 gap-x-2 md:gap-x-6 py-5 items-center">
              ...
              <div className="shrink-0 flex flex-col items-end">
                <a href={person.url}>
                  {/* use hyperlink icon */}
                  <FontAwesomeIcon icon={faExternalLink} className="text-2xl text-navbar hover:text-navbar-button mr-10" />
